...
# Only run this vignette if PG is available and > 9.5 (Where SKIP LOCKED appears)
if (RPostgres::postgresHasDefault()) {
    con <- dbConnect(RPostgres::Postgres())
    rs <- dbSendQuery(con, "SELECT current_setting('server_version_num')::INT >= 90500")
    knitr::opts_chunk$set(purl = dbFetch(rs)[1,1])
    dbClearResult(rs)
    dbDisconnect(con)
} else {
